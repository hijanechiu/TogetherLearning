...
public class MyController {	
	...
	@ResponseBody
	public String processPayment(HttpServletRequest request,@SessionAttribute Integer sid) {		
		String form = genAioCheckOutALL(request,sid);		
		System.out.printf("【ECPayServer.java】產生了讓消費者付款的表單：\n%s\n",form);	
		//輸出內容請參考最下面區段。		
		return form; 		
	}
	
	private String genAioCheckOutALL(HttpServletRequest request, Integer sid){	
		...
		obj.setCustomField1(Integer.toString(sid));         //抓當前學生資料
		obj.setCustomField2( request.getParameter("onekunit") );    //抓1000點的購買數量
		obj.setCustomField3( request.getParameter("fivekunit") );   //抓5000點的購買數量
		obj.setCustomField4( request.getParameter("tenkunit") );    //抓10000點的購買數量
